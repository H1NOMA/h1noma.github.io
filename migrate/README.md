...
|---|---|---|
| `push.sh` | Главная команда обслуживания: VAPID-ключи и функция notify-game, защита входа (GoTrue: лимит попыток по IP, пароль от 8 символов), **все четыре миграции базы по порядку, одной транзакцией** и в конце проверка «Итог: защита базы включена ✓». Повторный запуск безопасен. | `bash <(curl -fsS https://komikdnd.ru/migrate/push.sh)` (на вопрос о ключе — Enter) |
| `push.sh check` | Только проверка защиты базы и входа, ничего не меняет (правила kv, включён ли RLS на `kv`/`devs`, запрет переименования строк, команда, настройки входа). | `bash <(curl -fsS https://komikdnd.ru/migrate/push.sh) check` |
...
| `restore.sh` | Вернуть данные `kv` (всю таблицу или один ключ) из ежедневной копии; `accounts` — копия аккаунтов и состава команды в `/root`. | `bash <(curl -fsS https://komikdnd.ru/migrate/restore.sh)` — список дат |
...
| `2026-09-27-hardening.sql` | Права команды — у аккаунтов, а не у логинов; игроки без удаления и с потолком размера строк; ключ строки через API не меняется; тег только по правилам сайта; адрес аккаунта не меняется. | `push.sh` применяет сам |
...
пересоздают свои, более мягкие версии правил, а 09-27 последней возвращает строгие. Применяются они **одной
транзакцией**: у каждого файла снимаются свои строки `begin;`/`commit;`, обёртка — одна на все четыре. Снаружи
мягкого промежуточного состояния никто не видит, а ошибка или Ctrl+C в любом файле откатывает всё — база
остаётся как была. Поэтому в каждом файле `begin;` и `commit;` — отдельными строками (ровно по одной, `commit;` —
последней), и внутри нет ничего, что нельзя выполнять в транзакции. Любую правку прав — новым файлом
`migrate/ДАТА-*.sql` по тем же правилам, добавленным в конец списков `MIGS` в `push.sh` и `setup.sh`.
...
5. **Ключ строки не меняется.** Политика `UPDATE` смотрит на старую и новую строку, но не запрещала сменить сам
   ключ: игрок «переименовал» бы расписание или доску в свою пустую `comik:bm:<тег>` — и общая строка пропала бы
   без всякого `DELETE`. Триггер `kv_key_fixed`: для `anon`/`authenticated` смена ключа — ошибка `42501`. Upsert сайта
   (PostgREST: `on conflict (key) do update set key = excluded.key, …`) пишет тот же ключ — проходит; владельцу
   сервера (`postgres`, сервисный ключ `restore.sh`) триггер не мешает.
...
rollback to a;
update public.kv set key = 'comik:bm:roman' where key = 'comik:games:v1';       -- ERROR 42501 (ключ не меняется)
rollback;
```
